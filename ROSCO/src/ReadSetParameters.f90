    ...
    SUBROUTINE SetParameters(avrSWAP, accINFILE, size_avcMSG, CntrPar, LocalVar, objInst, PerfData, zmqVar, ErrVar)
        ...
        IF (LocalVar%iStatus == 0) THEN ! .TRUE. if we're on the first call to the DLL
            ...
            CALL CheckInputs(LocalVar, CntrPar, avrSWAP, ErrVar, size_avcMSG)
             ...
             IF (.not. allocated(LocalVar%REWS_f_IPC)) THEN
              Allocate(LocalVar%REWS_f_IPC(LocalVar%MaxBufferStep_REWS,5))
              LocalVar%REWS_f_IPC(:,:) = 0
             END IF
        
             IF (.not. allocated(LocalVar%REWS_f_Time_IPC)) THEN
             Allocate(LocalVar%REWS_f_Time_IPC(LocalVar%MaxBufferStep_REWS,5))
             LocalVar%REWS_f_Time_IPC(:,:) = 0
             END IF
             
             IF (.not. allocated(LocalVar%REWS_b_IPC)) THEN
              Allocate(LocalVar%REWS_b_IPC(5))
              LocalVar%REWS_b_IPC(:) = 0
             END IF
             
             IF (.not. allocated(LocalVar%LeadTime_1_IPC)) THEN
              Allocate(LocalVar%LeadTime_1_IPC(5))
              LocalVar%LeadTime_1_IPC(:) = 0
             END IF
             
             IF (.not. allocated(LocalVar%FF_Pitch_IPC)) THEN
              Allocate(LocalVar%FF_Pitch_IPC(5))
